...
export const registerUser = async (req: Request, res: Response) : Promise<Response>  => {
    try{
        ...
        const response: QueryResult = await pool.query('INSERT INTO users (name,email) VALUES ($1, $2)', [name, email]) // manda para o banco de dados um novo usuario com nome e email escritos no body
...
export const editUser = async (req: Request, res: Response) : Promise<Response>  => {
    try{
        ...
        const {name, email} = req.body;
        const response: QueryResult = await pool.query('UPDATE users SET name = $1, email = $2 WHERE id = $3', [name, email,idUpdate]) // manda para o banco de dados um novo usuario com nome e email escritos no body
        return res.status(200).json({
